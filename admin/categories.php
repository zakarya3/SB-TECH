...
$query1=$conn->prepare("SELECT COUNT(id_cat) FROM categorie;");
$query1->execute();
$res=$query1->fetchAll();
...
if (isset($_POST['ajouter'])) {
    $nom=$_POST['categ-name'];
    $file=$_FILES['image']['name'];
    if(isset($_FILES['image'])){
        $buffer = basename($_FILES["image"]["name"]);
            $tr = "fichiers/" . $buffer;
            move_uploaded_file($_FILES["image"]["tmp_name"], $tr);

            $query=$conn->prepare("INSERT INTO `categorie`(`nom_cat`, `image`) VALUES ('$nom','$file');");
            $query->execute(); 
}
}
...
if (isset($_POST['supprimer'])) {
    ...
    $query=$conn->prepare("DELETE FROM `categorie` WHERE nom_cat='$nom_cat';");
    $query->execute();
}
}else {
    ...
    <main>
            ...
            <div class="container-dashboard">
                <div class="con-7">
                    <div class="card-ov">
                        ...
                        <div class="number">
                            <?php echo $res[0]['COUNT(id_cat)'];?>
                        </div>
                        <div class="total">Totale categories</div>
                    </div>
                </div>
                <div class="con-7">
        ...
        <section id="Add-cat">
            ...
            <form action method="POST" enctype="multipart/form-data">
                <div>
                    <label for="marqName">Nom de categorie</label>
                    <input required type="text" placeholder=" Entrer le nom du categorie ici " id="marqName"
                        name="categ-name" value="">
                </div>
                <div>
                        <label for="prodPic">Photo de la categorie</label>
                        <input required style="margin-top: 1em;" type="file" name="image">
                    </div>
        ...
        <section id="Add-cat">
            <div class="title">Supprimer une categorie</div>
            <form action="" method="POST">
                <div>
                    <label for="marqId">Nom de categorie</label>
                    <input required type="text" placeholder=" Entrer nom de categorie  ici " id="marqId" name="nom-cat"
                        value="">
                </div>
