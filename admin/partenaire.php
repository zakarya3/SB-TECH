...
$query1=$conn->prepare("SELECT COUNT(id_part) FROM `partner`;");
$query1->execute();
$res=$query1->fetchAll();
...
if(isset($_FILES['photo'])){
            $buffer3 = basename($_FILES["photo"]["name"]);
            $tr3 = "fichiers/" . $buffer3;
            move_uploaded_file($_FILES["photo"]["tmp_name"], $tr3);
            $query=$conn->prepare("INSERT INTO `partner`(`nom_part`, `image`) VALUES ('$nom','$file');");
            $query->execute();  
}
}
...
if (isset($_POST['supprimer'])) {
    $nom_part=$_POST['nom-part'];
    $query=$conn->prepare("DELETE FROM `partner` WHERE nom_part='$nom_part';");
    $query->execute();
}
}else {
...
<body>
    ...
    <main>
            ...
            <div class="container-dashboard">
                <div class="con-7">
                    <div class="card-ov">
                        ...
                        <div class="number">
                        <?php echo $res[0]['COUNT(id_part)'];?>
                        </div>
                        <div class="total">Totale Partenaires</div>
                    </div>
                </div>
                <div class="con-7">
        ...
        <section id="Add-cat">
            <div class="title">Ajouter une Partenaire</div>
            <form action method="POST" enctype="multipart/form-data">
                <div>
                    <label for="marqName">Nom de Partenaire</label>
                    <input required type="text" placeholder=" Entrer le nom du Partenaire ici " id="marqName"
                        name="part-name" value="">
                </div>

                <div>
                    <label for="marqPic">Photo de Partenaire</label>
                    <input required style="margin-top: 1em;" type="file" name="photo" id="marqPic" accept="image/*"
                        onchange="loadFile(event)">
                    <div class="con">
                        <img class="cover" id="output" />
        ...
        <section id="Add-cat">
            <div class="title">Supprimer une Partenaire</div>
            <form action="" method="POST">
                <div>
                    <label for="marqId">Nom de Partenaire</label>
                    <input required type="text" placeholder=" Entrer nom de Partenaire  ici " id="marqId" name="nom-part"
                        value="">
                </div>
